...
(* compares to points *)
let equal p1 p2 = Float.compare p1.x p2.x == 0 && Float.compare p1.y p2.y == 0

(* compares two floats *)
let equalf ?(e = ll_epsilon) f1 f2 = Float.abs (f1 -. f2) <= e

let approximatelyEqual ?(e = ll_epsilon) p1 p2 =
  equalf p1.x p2.x ~e && equalf p1.y p2.y ~e
